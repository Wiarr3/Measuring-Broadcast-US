...
# Tworzymy tekst do wyświetlenia na mapie
metrics['text'] = metrics.apply(lambda row: f"Location: {row['location']}<br> Error Count: {row['error_count']}", axis=1)

# Tworzymy mapkę z Plotly
fig = px.scatter_geo(metrics,
    lat='lat',
    lon='lon',
    text=metrics['text'],  # Dodajemy pełny tekst do wyświetlenia
    ...
    title="🌍 Error Count i Lokalizacje",
)
